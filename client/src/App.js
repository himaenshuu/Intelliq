...
function App() {
    ...
    const onDrop = async (acceptedFiles) => {
        ...
        try {
            const endpoint = activeTab === 1 ? '/api/process-pdf' : '/api/process-image';
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({
        onDrop,
        accept: activeTab === 1 ? { 'application/pdf': ['.pdf'] } : { 'image/*': ['.png', '.jpg', '.jpeg'] },
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (file) {
                ...
                const endpoint = isPDF ? '/api/process-pdf' : '/api/process-image';
        ...
        <div className={`App ${isDarkMode ? 'dark-theme' : 'light-theme'}`}>
            ...
            <footer className="App-footer">
                <div className="footer-content">
                    <p>© {new Date().getFullYear()} AI Quiz Solver. All rights reserved.</p>
                </div>
            </footer>
        </div>
    );
